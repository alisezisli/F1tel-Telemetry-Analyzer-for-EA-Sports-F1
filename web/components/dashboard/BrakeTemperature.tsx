...
const CORNERS = [
  { key: "brake_temp_fl" as const, label: "FL", color: "#3b82f6" },
  { key: "brake_temp_fr" as const, label: "FR", color: "#22c55e" },
  { key: "brake_temp_rl" as const, label: "RL", color: "#f59e0b" },
  { key: "brake_temp_rr" as const, label: "RR", color: "#a855f7" },
];
...
export function BrakeTemperature({ frames, groupId }: Props) {
  const t = useT();
  ...
  const handleDownload = useCallback(() => {
    ...
    if (!url) return;
    const a = document.createElement("a"); a.href = url; a.download = "brake-temperature.png"; a.click();
  }, []);
  ...
  const option = useMemo(() => ({
    ...
    tooltip: {
      ...
      formatter: (params: unknown) => {
        ...
        const lines = p.map((item) => `${item.seriesName}: ${item.value[1].toFixed(0)}°C`);
