...
export function Steering({ frames, laps, groupId, selectedLaps, availableLaps, onToggleLap }: Props) {
  const t = useT();
  ...
  const handleDownload = useCallback(() => {
    ...
    if (!url) return;
    const a = document.createElement("a"); a.href = url; a.download = "steering.png"; a.click();
  }, []);
  ...
  const option = useMemo(() => ({
    ...
    yAxis: {
      ...
      min: -1,
      max: 1,
      axisLabel: { fontSize: 10, color: "#a1a1aa", formatter: (v: number) => v.toFixed(1) },
      axisLine: { show: false },
      splitLine: { lineStyle: { color: "#262626" } },
    },
    series: [
      {
        ...
        markLine: {
          silent: true,
          symbol: "none",
          lineStyle: { color: "#3f3f46", type: "dashed" as const, width: 1 },
          data: [{ yAxis: 0 }],
          label: { show: false },
        },
        animation: false,
      ...
      ...selectedLaps.map((lap, idx) => {
        ...
        return {
          name: `L${lap}`,
          type: "line" as const,
          data: points.map(([dist, f]) => [Number(dist), parseFloat(f.steering.toFixed(3))]),
    ...
    tooltip: {
      ...
      formatter: (params: unknown) => {
        const p = (params as Array<{ seriesName: string; value: [number, number] }>)
          .filter((item) => item.seriesName !== "_zero");
        const dist = p[0]?.value[0] ?? "";
        const lines = p.map((item) => `${item.seriesName}: ${item.value[1].toFixed(2)}`);
